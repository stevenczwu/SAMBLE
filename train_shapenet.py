...
def train(local_rank, config, random_seed,
    ...
    if config.wandb.enable and rank == 0:
        ...
        os.system(f'cp ./models/seg_model.py {save_dir}{time_label}_{run.id}/models/seg_model.py')
        os.system(f'cp ./models/seg_block.py {save_dir}{time_label}_{run.id}/models/seg_block.py')
        ...
        os.system(f'cp ./models/upsample.py {save_dir}{time_label}_{run.id}/models/upsample.py')
        ...
        os.system(f'cp ./train_shapenet.py {save_dir}{time_label}_{run.id}/train_shapenet.py')
        with open(f'{save_dir}{time_label}_{run.id}/random_seed_{random_seed}.txt', 'w') as f:
            f.write('')
    ...
    if config.datasets.dataset_name == 'shapenet_Yi650M':
        ...
        train_set, validation_set, trainval_set, test_set = dataloader.get_shapenet_dataset_Normal(config.datasets,
                                                                                                   config.train.dataloader,
                                                                                                   config.train.dataloader.vote)
    else:
        raise ValueError('Not implemented!')
    ...
    for epoch in range(config.train.epochs):
        ...
        if not (epoch + 1) % config.train.validation_freq:
            ...
            with torch.no_grad():
                pbar_val = pkbar.Pbar(name=f'\nValidating, please wait...', target=len(validation_loader))
                for i_val, (samples, seg_labels, cls_label) in enumerate(validation_loader):
