...
def test(local_rank, config):
    ...
    if config.test.suffix.enable:
        ...
        backup_path = (
            f"/data/users/wan/artifacts/{config.wandb.name}_{config.test.suffix.remark}"
        )
    else:
    ...
    with torch.no_grad():
        ...
        for i, (samples, seg_labels, cls_label) in enumerate(test_loader):
            ...
            if config.test.dataloader.vote.enable:
                ...
                pred_novo_gather_list = [
                    torch.empty_like(preds_novo).to(device)
                ...
                if rank == 0:
                    ...
                    pred_novo_list.append(
                        torch.max(preds_novo.permute(0, 2, 1), dim=2)[1]
                        .detach()
                        .cpu()
                        .numpy()
                    )
            ...
            if config.test.visualize_combine.enable:
                ...
                if rank == 0:
                    ...
                    data_dict = {
                        ...
                        "samples": torch.concat(sample_gather_list, dim=0).permute(
                            0, 2, 1
                        ),  # (B,N,3)
                        ...
                        # (B,)
                        "seg_predictions": torch.argmax(
                            torch.concat(pred_gather_list, dim=0), dim=1
                        ),  # (B,)
                        "seg_ground_truth": torch.argmax(
                            torch.concat(seg_label_gather_list, dim=0), dim=1
                        ),
                        "config": config,
            ...
            if rank == 0:
                ...
                samples = torch.concat(
                    sample_gather_list, dim=0
                )  # samples.shape == (B, C, N)
                sample_list.append(
                    samples[:, :3, :].permute(0, 2, 1).detach().cpu().numpy()
                )
