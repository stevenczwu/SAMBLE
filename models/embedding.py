...
class EdgeConv(nn.Module):
    def __init__(self, config_embedding, layer):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(conv1_channel_in, conv1_channel_out, kernel_size=1, bias=False),
            nn.BatchNorm2d(conv1_channel_out),
            nn.LeakyReLU(negative_slope=0.2),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(conv2_channel_in, conv2_channel_out, kernel_size=1, bias=False),
...
class STN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.max(dim=-1, keepdim=False)[
            0
        ]  # (B, 128, num_points, k) -> (B, 128, num_points)
