...
class UpSample(nn.Module):
    ...
    def forward(self, pcd_up, pcd_down, pcd_up_xyz):
        (points_select, idx_select, points_select_xyz), (
            points_drop,
            idx_drop,
        ) = pcd_down
...
class UpSampleInterpolation(nn.Module):
    def __init__(self, config_upsample, layer):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(q_in, v_out, 1, bias=False),
            nn.BatchNorm1d(v_out),
        ...
        self.res_conv = nn.Sequential(
            nn.Conv1d(2 * v_out, v_out, 1, bias=False),
            nn.BatchNorm1d(v_out),
            nn.LeakyReLU(negative_slope=0.2),
        )
    ...
    def interpolate(
        ...
        pcd_up_xyz,
        points_select_xyz,
        distance_type="feature",
        K=3,
    ):
        points_select_conv = self.conv(
            points_select
        )  # points_select_conv.shape == (B, C, M)
        if distance_type == "feature":
            neighbors, _, d_neighbors = ops.select_neighbors_interpolate(
                pcd_up, points_select, points_select_conv, K=K
            )
        elif distance_type == "xyz":
            neighbors, _, d_neighbors = ops.select_neighbors_interpolate(
                pcd_up_xyz, points_select_xyz, points_select_conv, K=K
            )
        else:
            raise ValueError(
                f"upsample interpolation distance type can only be feature or xyz! Got: {distance_type}"
            )
        ...
        interpolated_points = torch.sum(
            neighbors * weights.unsqueeze(dim=1), dim=-1
